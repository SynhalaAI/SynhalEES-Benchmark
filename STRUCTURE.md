...
│   ├── robots.txt                      # Static: crawler allow + sitemap pointer
│   ├── sitemap.xml                     # Static: single-URL sitemap (no dates, never drifts)
│   ├── 404.html                        # Static: branded 404 with link back to leaderboard
│   ├── seo-structured-data.json        # GENERATED by build_leaderboard.py (Dataset JSON-LD, --check guarded)
│   └── assets/
│       ├── style.css                            # SynhalaAI theme (logo navy #2B3044 + red #C62828)
│       ├── app.js                               # Vanilla JS: sorting, modality tabs, pillar filter, radar chart
│       ├── chart.js                             # AA-style comparison bar chart + Pareto frontier scatter (Bars/Pareto view switch, cost/latency/tokens for bars + Pareto axes (auto log scale), metric tabs, org filter, model picker, max 25 bars, efficient/inefficient dominance shading)
