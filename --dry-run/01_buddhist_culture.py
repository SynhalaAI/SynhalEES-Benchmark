# SynhalEES Kaggle task: Buddhist Culture (text). Auto-generated by kaggle/generate_tasks.py.
...
PILLAR = '01_buddhist_culture'  # None -> all pillars
MODALITY = "text"
...
@kbench.task(name="synhalees_01_buddhist_culture_item", store_task=False)
...
@kbench.task(name="synhalees_01_buddhist_culture", description="SynhalEES pillar 01_buddhist_culture: native Sinhala text accuracy.")
def synhalees_task(llm) -> float:
    '''Evaluate Buddhist Culture (text); return accuracy in [0, 1].'''
    import pandas as pd

    rows = _rows()
    if not rows:
        raise ValueError(
            "No runnable text rows for Buddhist Culture yet -- media files may "
