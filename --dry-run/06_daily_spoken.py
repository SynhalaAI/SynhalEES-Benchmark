# SynhalEES Kaggle task: Daily Spoken (text). Auto-generated by kaggle/generate_tasks.py.
...
PILLAR = '06_daily_spoken'  # None -> all pillars
MODALITY = "text"
...
@kbench.task(name="synhalees_06_daily_spoken_item", store_task=False)
...
@kbench.task(name="synhalees_06_daily_spoken", description="SynhalEES pillar 06_daily_spoken: native Sinhala text accuracy.")
def synhalees_task(llm) -> float:
    '''Evaluate Daily Spoken (text); return accuracy in [0, 1].'''
    import pandas as pd

    rows = _rows()
    if not rows:
        raise ValueError(
            "No runnable text rows for Daily Spoken yet -- media files may "
