# SynhalEES Kaggle task: All Pillars (vision) (vision). Auto-generated by kaggle/generate_tasks.py.
...
@kbench.task(name="synhalees_vision_item", store_task=False)
...
@kbench.task(name="synhalees_vision", description="SynhalEES vision modality across all 15 pillars.")
def synhalees_task(llm) -> float:
    '''Evaluate All Pillars (vision) (vision); return accuracy in [0, 1].'''
    import pandas as pd

    rows = _rows()
    if not rows:
        raise ValueError(
            "No runnable vision rows for All Pillars (vision) yet -- media files may "
