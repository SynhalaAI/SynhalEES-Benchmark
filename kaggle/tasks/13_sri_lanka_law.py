# SynhalEES Kaggle task: Sri Lanka Law (text). Auto-generated by kaggle/generate_tasks.py.
...
PILLAR = '13_sri_lanka_law'  # None -> all pillars
MODALITY = "text"
...
@kbench.task(name="synhalees_13_sri_lanka_law_item", store_task=False)
...
@kbench.task(name="synhalees_13_sri_lanka_law", description="SynhalEES pillar 13_sri_lanka_law: native Sinhala text accuracy.")
def synhalees_task(llm) -> float:
    '''Evaluate Sri Lanka Law (text); return accuracy in [0, 1].'''
    import pandas as pd

    rows = _rows()
    if not rows:
        raise ValueError(
            "No runnable text rows for Sri Lanka Law yet -- media files may "
