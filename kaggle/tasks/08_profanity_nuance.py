# SynhalEES Kaggle task: Profanity Nuance (text). Auto-generated by kaggle/generate_tasks.py.
...
PILLAR = '08_profanity_nuance'  # None -> all pillars
MODALITY = "text"
...
@kbench.task(name="synhalees_08_profanity_nuance_item", store_task=False)
...
@kbench.task(name="synhalees_08_profanity_nuance", description="SynhalEES pillar 08_profanity_nuance: native Sinhala text accuracy.")
def synhalees_task(llm) -> float:
    '''Evaluate Profanity Nuance (text); return accuracy in [0, 1].'''
    import pandas as pd

    rows = _rows()
    if not rows:
        raise ValueError(
            "No runnable text rows for Profanity Nuance yet -- media files may "
