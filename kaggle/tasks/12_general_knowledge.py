# SynhalEES Kaggle task: General Knowledge (text). Auto-generated by kaggle/generate_tasks.py.
...
PILLAR = '12_general_knowledge'  # None -> all pillars
MODALITY = "text"
...
@kbench.task(name="synhalees_12_general_knowledge_item", store_task=False)
...
@kbench.task(name="synhalees_12_general_knowledge", description="SynhalEES pillar 12_general_knowledge: native Sinhala text accuracy.")
def synhalees_task(llm) -> float:
    '''Evaluate General Knowledge (text); return accuracy in [0, 1].'''
    import pandas as pd

    rows = _rows()
    if not rows:
        raise ValueError(
            "No runnable text rows for General Knowledge yet -- media files may "
