# SynhalEES Kaggle task: Kavi Sindu (text). Auto-generated by kaggle/generate_tasks.py.
...
PILLAR = '04_kavi_sindu'  # None -> all pillars
MODALITY = "text"
...
@kbench.task(name="synhalees_04_kavi_sindu", description="SynhalEES pillar 04_kavi_sindu: native Sinhala text accuracy.")
def synhalees_task(llm) -> float:
    '''Evaluate Kavi Sindu (text); return accuracy in [0, 1].'''
    import pandas as pd

    rows = _rows()
    if not rows:
        raise ValueError(
            "No runnable text rows for Kavi Sindu yet -- media files may "
