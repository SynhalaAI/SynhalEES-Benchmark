# SynhalEES Kaggle task: Figurative Sinhala (text). Auto-generated by kaggle/generate_tasks.py.
...
@kbench.task(name="synhalees_07_figurative_sinhala_item", store_task=False)
...
@kbench.task(name="synhalees_07_figurative_sinhala", description="SynhalEES pillar 07_figurative_sinhala: native Sinhala text accuracy.")
def synhalees_task(llm) -> float:
    '''Evaluate Figurative Sinhala (text); return accuracy in [0, 1].'''
    import pandas as pd

    rows = _rows()
    if not rows:
        raise ValueError(
            "No runnable text rows for Figurative Sinhala yet -- media files may "
