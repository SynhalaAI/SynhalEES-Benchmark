# SynhalEES Kaggle task: Sinhala Grammar (text). Auto-generated by kaggle/generate_tasks.py.
...
PILLAR = '05_sinhala_grammar'  # None -> all pillars
MODALITY = "text"
...
@kbench.task(name="synhalees_05_sinhala_grammar_item", store_task=False)
...
@kbench.task(name="synhalees_05_sinhala_grammar", description="SynhalEES pillar 05_sinhala_grammar: native Sinhala text accuracy.")
def synhalees_task(llm) -> float:
    '''Evaluate Sinhala Grammar (text); return accuracy in [0, 1].'''
    import pandas as pd

    rows = _rows()
    if not rows:
        raise ValueError(
            "No runnable text rows for Sinhala Grammar yet -- media files may "
