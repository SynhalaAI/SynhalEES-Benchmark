# SynhalEES Kaggle task: Classical Literature (text). Auto-generated by kaggle/generate_tasks.py.
...
@kbench.task(name="synhalees_03_classical_literature_item", store_task=False)
...
@kbench.task(name="synhalees_03_classical_literature", description="SynhalEES pillar 03_classical_literature: native Sinhala text accuracy.")
def synhalees_task(llm) -> float:
    '''Evaluate Classical Literature (text); return accuracy in [0, 1].'''
    import pandas as pd

    rows = _rows()
    if not rows:
        raise ValueError(
            "No runnable text rows for Classical Literature yet -- media files may "
