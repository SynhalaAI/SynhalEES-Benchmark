# SynhalEES Kaggle task: Numbers Maths (text). Auto-generated by kaggle/generate_tasks.py.
...
PILLAR = '15_numbers_maths'  # None -> all pillars
MODALITY = "text"
...
@kbench.task(name="synhalees_15_numbers_maths_item", store_task=False)
...
@kbench.task(name="synhalees_15_numbers_maths", description="SynhalEES pillar 15_numbers_maths: native Sinhala text accuracy.")
def synhalees_task(llm) -> float:
    '''Evaluate Numbers Maths (text); return accuracy in [0, 1].'''
    import pandas as pd

    rows = _rows()
    if not rows:
        raise ValueError(
            "No runnable text rows for Numbers Maths yet -- media files may "
