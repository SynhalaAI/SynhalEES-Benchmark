# SynhalEES Kaggle task: Pali Gatha (text). Auto-generated by kaggle/generate_tasks.py.
...
PILLAR = '02_pali_gatha'  # None -> all pillars
MODALITY = "text"
...
@kbench.task(name="synhalees_02_pali_gatha_item", store_task=False)
...
@kbench.task(name="synhalees_02_pali_gatha", description="SynhalEES pillar 02_pali_gatha: native Sinhala text accuracy.")
def synhalees_task(llm) -> float:
    '''Evaluate Pali Gatha (text); return accuracy in [0, 1].'''
    import pandas as pd

    rows = _rows()
    if not rows:
        raise ValueError(
            "No runnable text rows for Pali Gatha yet -- media files may "
